...
import Product from '../models/Product.js'; // Ensure this points to your general Product model
...
router.get('/:id', async (req, res) => {
  try {
    ...
    console.error('Error fetching single general product:', error);
    res.status(500).json({
      message: 'Failed to fetch general product',
      error: error.message
    });
  }
});
